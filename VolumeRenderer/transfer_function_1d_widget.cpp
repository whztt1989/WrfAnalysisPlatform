...
#include "qrender_hover_points.h"
...
void TransferFunction1DWidget::SetData( unsigned int data_size, short* data ){
    ...
    GetMinMaxValue(data_size, data, min_value_, max_value_);

    this->SetViewWindow((max_value_ - min_value_) / 2, max_value_ - min_value_);
}
...
void TransferFunction1DWidget::SetData( unsigned int data_size, float* data ){
...
template < typename T >
void TransferFunction1DWidget::GetMinMaxValue(unsigned int data_size, T* data, float& min_value, float& max_value){
    min_value = 1e10;
    max_value = -1e10;
...
void TransferFunction1DWidget::SetViewWindow( double window_level, double window_width ){
	...
	case TYPE_SHORT:
		...
		break;
    case TYPE_FLOAT:
        GenerateHistogramValues(data_size_, (float*)data_);
        break;
	default:
		break;
	}
...
void TransferFunction1DWidget::GenerateLineColorAndAlphaValue(){
	...
	for ( int i = 0; i < histogram_bin_size_; ++i )
	{
		...
		float alpha = ((0.9 - (double)points.at(index).y() / this->height()) * (1.0 - rate) + (0.9 - (double)points.at(index + 1).y() / this->height()) * rate) / 0.8;
        tf_value_array_[i * 4 + 3] = (exp(alpha) - 1) / 1.7182818;
...
void TransferFunction1DWidget::GenerateHistogramValues( unsigned int data_size, T* data ){
    ...
    float temp_min_value, temp_max_value;
    if ( data_type_ == TYPE_FLOAT ){
        temp_max_value = (view_max_value_ - min_value_) / (max_value_ - min_value_);
        temp_min_value = (view_min_value_ - min_value_) / (max_value_ - min_value_);
    } else {
        temp_max_value = view_max_value_;
        temp_min_value = view_min_value_;
    }

	for ( unsigned int i = 0; i < data_size; ++i ){
		int bin_index = (int)((data[i] - temp_min_value) / (temp_max_value - temp_min_value) * histogram_bin_size_);
